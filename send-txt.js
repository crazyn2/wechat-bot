...
const getDateTime = (date) => {
  ...
  const dateStr = `${year}年${month}月${day}日 ${hours}:${minutes}:${seconds}`;
  return dateStr;
}
 ...
 * send txt message
 */
async function send_txt_msg2(targetId,msg)
{
  const sendmsg = msg+"\n发送时间："+getDateTime(new Date());
  const jpara={
    id:getid(),
    type:TXT_MSG,
    wxid:targetId,//roomid或wxid,必填
    roomid:'null',//此处为空
    content:sendmsg,
    nickname:"null",//此处为空
    ext:'null'//此处为空
...
async function main()
{
	...
	if(args.length > 2){
		console.log("0 < args < 2");
		return;
	}
	const j = await send_txt_msg2(args[0],args[1])
